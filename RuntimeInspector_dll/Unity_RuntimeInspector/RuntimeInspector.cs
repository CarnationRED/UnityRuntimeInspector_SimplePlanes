...
namespace RuntimeInspectorNamespace
{
    ...
    {
        #region Plugin workaround added by CarnationRED
        /// <summary>
        /// to load ObjectReferencePicker, ColorPicker, DraggedReferenceItem and Tooltip from assetbundle, instead of using "Resource.Load"
        /// </summary>
        internal static void LoadResources()
        {
            AssetBundle assetBundle = AssetBundle.LoadFromFile(Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "runtimeinspector_colorpicker.bundle"));
            ColorPicker.Instance = Instantiate(assetBundle.LoadAsset<GameObject>("ColorPicker")).GetComponent<ColorPicker>();
            ObjectReferencePicker.Instance = Instantiate(assetBundle.LoadAsset<GameObject>("ObjectReferencePicker")).GetComponent<ObjectReferencePicker>();
            ColorPicker.Instance.gameObject.SetActive(false);
            ObjectReferencePicker.Instance.gameObject.SetActive(false);
            ...
            GetDraggedReferenceItem(assetBundle);
            GetTooltip(assetBundle);
            assetBundle.Unload(false);
        }
        static DraggedReferenceItem cacheDraggedReferenceItem;
        ...
        static Tooltip cacheTooltip;
        internal static Tooltip GetTooltip(AssetBundle assetBundle = null)
        {
            if (!cacheTooltip)
            {
                GameObject go;
                if (!assetBundle)
                {
                    ...
                    go = Instantiate(assetBundle.LoadAsset<GameObject>("Tooltip")).gameObject;
                    go.SetActive(false);
                    assetBundle.Unload(false);
                    return cacheTooltip = go.GetComponent<Tooltip>();
                }
        ...
        /// <summary>
        /// have to init these manually, dont know why
        /// </summary>
        internal static RuntimeInspectorSettings InitRuntimeInspectorSettings(RuntimeInspectorSettings ris)
        {
            ris.HiddenVariables = new VariableSet[] {
            new VariableSet()
            {
                type=typeof(Object),
...
#pragma warning restore 0649
        ...
        protected override void Awake()
        {
            ...
            {
                if (settings[i].HiddenVariables == null) settings[i]=InitRuntimeInspectorSettings(settings[i]);
                VariableSet[] hiddenVariablesForTypes = settings[i].HiddenVariables;
                if (hiddenVariablesForTypes != null) for (int j = 0; j < hiddenVariablesForTypes.Length; j++)
                    {
                ...
                if (exposedVariablesForTypes != null) for (int j = 0; j < exposedVariablesForTypes.Length; j++)
                    {
